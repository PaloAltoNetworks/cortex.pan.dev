...
const getDownloadURL = (_ref9, _ref10) => {
  ...
  return makeAuthorizedRequest("".concat(rootURL, "/origin/").concat(sha).concat(t && Object.keys(t).length > 0 ? "?nf_resize=".concat(t.nf_resize, "&w=").concat(t.w, "&h=").concat(t.h) : '')).then(res => res.ok ? res : Promise.reject(res)).then(res => res.blob()).then(blob => URL.createObjectURL(blob)).catch(err => console.error(err) || Promise.resolve(''));
};

const getResourceDownloadURLArgs = (clientConfig, objects) => {
  return Promise.resolve(objects.map((_ref11) => {
    let {
      sha
    } = _ref11;
    return [sha, {
