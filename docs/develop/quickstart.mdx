...
<Tabs
defaultValue={typeof window !== 'undefined' ? localStorage.getItem('defaultLanguage') : 'python'}
values={[
...
Perhaps the easiest way to get started is by leveraging a Developer Token provided by the [API Explorer's](https://app.apiexplorer.rocks) token redemption service. See [Developer Tokens](developer_tokens) for details.
